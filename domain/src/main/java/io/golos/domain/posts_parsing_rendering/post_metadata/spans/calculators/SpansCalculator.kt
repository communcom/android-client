...
abstract class SpansCalculator<T>(spansReader: SpansWorkerRead) {
    ...
    private fun calculateOperations(span: SpanInfo<T>, intersections: SpansIntersections<T>): List<SpanOperation> {
        ...
        if(fullIntersection != null) {
            ...
            result.add(
                DeleteSpanOperation(
                    spansInText.getValue(fullIntersection)
                )
            )
        ...
        if(oldFullOutside != null) {
            ...
            result.add(
                DeleteSpanOperation(
                    spansInText.getValue(oldFullOutside)
                )
            )
        ...
        intersections.spansInsideFull.forEach { insideSpan ->
            result.add(
                DeleteSpanOperation(
                    spansInText.getValue(insideSpan)
                )
            )
        }
        ...
        intersections.spansInsideLeft.forEach { oldLeftSpan ->
            createSpanInfo(oldLeftSpan.area.first..span.area.first, oldLeftSpan.displayValue).let { newLeftSpan ->
                result.add(
                    DeleteSpanOperation(
                        spansInText.getValue(oldLeftSpan)
                    )
                )
        ...
        intersections.spansInsideRight.forEach { oldRightSpan ->
            createSpanInfo(span.area.last..oldRightSpan.area.last, oldRightSpan.displayValue).let { newRightSpan ->
                result.add(
                    DeleteSpanOperation(
                        spansInText.getValue(oldRightSpan)
                    )
                )
