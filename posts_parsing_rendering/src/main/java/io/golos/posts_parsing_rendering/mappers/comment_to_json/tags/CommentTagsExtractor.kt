package io.golos.posts_parsing_rendering.mappers.comment_to_json.tags
...
object CommentTagsExtractor {
    ...
    fun extract(text: String): List<StringPart> {
        ...
        if(startIndex == 1) {
            result.add(StringPart(parts[0], false))
        }

        for(i in startIndex until parts.size) {
            if(parts[i].isNotEmpty()) {
                splitString(parts[i], result)
            }
        }
    ...
    private fun splitString(s: String, partsList: MutableList<StringPart>) {
        s.indexOf(" ")
            .let {
                if(it == -1) {
                    partsList.add(StringPart(s, true))
                } else {
                    partsList.add(StringPart(s.substring(0 until it), true))
                    partsList.add(StringPart(s.substring(it), false))
                }
            }
    }
}