            ...
            jsonContent.getJSONObject(i)
                .also {
                    val block = when(val type = it.getType()) {
                        BlockType.IMAGE -> mappersFactory.getMapper<ImageMapper>(ImageMapper::class).map(it)
                        BlockType.VIDEO -> mappersFactory.getMapper<VideoMapper>(VideoMapper::class).map(it)
                        BlockType.WEBSITE -> mappersFactory.getMapper<WebsiteMapper>(WebsiteMapper::class).map(it)
