...
class MainActivity : ActivityBaseMVVM<ActivityMainBinding, MainViewModel>() {
    ...
    override fun onNewIntent(intent: Intent) {
        ...
        if(intent.action == IntentConstants.ACTION_OPEN_NOTIFICATION) {
            getDashboardFragment()?.processIntent(intent)
        }
    }
    ...
    private fun getDashboardFragment(): DashboardFragment? =
        supportFragmentManager.findFragmentById(R.id.mainNavHost)?.childFragmentManager?.fragments?.get(0) as? DashboardFragment
}
