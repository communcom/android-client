...
class WelcomeActivity : ActivityBase(), SplashAnimatorTarget {
    ...
    private fun navigateToMainScreen() {
        ...
        if (intent?.action == Intent.ACTION_SEND) {
            val args = EditorPageFragment.Args()
            if ("text/plain" == intent.type) {
                args.sharedText = intent.getStringExtra(Intent.EXTRA_TEXT)
            } else if (intent.type?.startsWith("image/") == true) {
                args.sharedImage = intent.getParcelableExtra<Parcelable>(Intent.EXTRA_STREAM) as Uri?
            }
            startActivity(EditorPageActivity.getIntent(this,args).apply {
                ...
                when(oldIntent.action) {
                    ...
                    IntentConstants.ACTION_OPEN_NOTIFICATION -> {
                        ...
                        if(oldIntent.extras!!.containsKey(IntentConstants.POST_CONTENT_ID)) {
                            newIntent.putExtra(IntentConstants.POST_CONTENT_ID,
                                oldIntent.getParcelableExtra<ContentIdDomain>(IntentConstants.POST_CONTENT_ID))
                        }
                        if(oldIntent.extras!!.containsKey(IntentConstants.USER_ID)) {
                            newIntent.putExtra(IntentConstants.USER_ID,
                                oldIntent.getParcelableExtra<UserIdDomain>(IntentConstants.USER_ID))
                        }
                        if(oldIntent.extras!!.containsKey(IntentConstants.WALLET)) {
                            newIntent.putExtra(IntentConstants.WALLET,
                                oldIntent.getBooleanExtra(IntentConstants.WALLET, true))
                        }
                    }
                }
            }
