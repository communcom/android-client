...
constructor(
    ...
    data class Profile(val metadata: UserMetadataModel, val isActiveUserProfile: Boolean) {
        companion object {
            val EMPTY =
                Profile(UserMetadataModel.empty, false)
            val NOT_FOUND =
                Profile(UserMetadataModel.empty, true)
        }
    }
...
private fun MediatorLiveData<QueryResult<OldProfileViewModel.Profile>>.postProfileData(
    ...
    when (metadataResult) {
        ...
        is QueryResult.Error -> when {
            metadataResult.error is AppError.NotFoundError ->
                postValue(QueryResult.Error(metadataResult.error,
                    OldProfileViewModel.Profile.NOT_FOUND
                ))
            else ->
                postValue(QueryResult.Error(metadataResult.error,
                    OldProfileViewModel.Profile.EMPTY
                ))
        }
        is QueryResult.Loading -> postValue(QueryResult.Loading(OldProfileViewModel.Profile.EMPTY))
    }
}