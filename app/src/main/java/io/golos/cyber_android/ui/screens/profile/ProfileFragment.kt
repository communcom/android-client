...
import io.golos.cyber_android.ui.screens.profile.edit.bio.EditBioFragment
...
//            when (it) {
//                is QueryResult.Success -> bindProfile(it.originalQuery)
//                is QueryResult.Error -> onError()
//                is QueryResult.Loading -> onLoading()
//            }
//        })
//
//        viewModel.getMyUserLiveData.observe(this, Observer {
//            updateControlsVisibility(it)
//        })
    ...
    private fun updatePersonalData(profile: UserMetadataModel) {
        if (profile.personal.coverUrl.isNullOrBlank())
            Glide.with(requireContext()).load(R.color.blue).into(cover)
        else Glide.with(requireContext()).load(profile.personal.coverUrl).into(cover)
        ...
        if (profile.personal.avatarUrl.isNullOrBlank()) {
            ...
            Glide.with(requireContext()).load(profile.personal.avatarUrl).into(avatar)
    ...
    private fun updateEditBioControls(profile: UserMetadataModel) {
        listOf(bio, editBio).forEach {
            it.setOnClickListener {
                startActivity(
                    EditBioActivity.getIntent(
                        requireContext(),
                        EditBioFragment.Args(
