...
open class WalletSendPointsAdapter(
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolderBase<WalletSendPointsListItemEventsProcessor, VersionedListItem> =
        when(viewType) {
            SEND_POINTS -> WalletSendPointsViewHolder(
                parent
            )
            LOADING -> WalletSendPointsLoadingViewHolder(
                parent
            )
            RETRY -> WalletSendPointsRetryViewHolder(
                parent
            )
            NO_DATA -> WalletSendPointsNoDataViewHolder(
                parent
            )
