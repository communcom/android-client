 ...
 */
abstract class AbstractFeedWithCommentsViewModel<out T : PostFeedUpdateRequest, E: DiscussionEntity, M: DiscussionModel>(
    feedUseCase: AbstractFeedUseCase<out T, E, M>,
    voteUseCase: VoteUseCase,
    private val posterUseCase: DiscussionPosterUseCase
) :
    AbstractFeedViewModel<T, E, M>(feedUseCase, voteUseCase) {
    ...
    fun sendComment(post: DiscussionModel, comment: String) {
        if (validate(comment)) {
            val postRequest = CommentCreationRequestModel(comment, post.contentId, emptyList())
